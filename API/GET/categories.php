<?php
	...
	if (isset($_GET["empty"]) && $_GET["empty"] === "false") {
		...
		$categories = $categories = $db->query("SELECT C.CATEGORY_ID, C.CATEGORY_NAME, COUNT(P.PRODUCT_ID) AS COUNT FROM CATEGORIES C LEFT JOIN PRODUCTS P ON C.CATEGORY_ID = P.CATEGORY_ID GROUP BY C.CATEGORY_ID");
	}
	else {
